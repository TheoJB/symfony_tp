...
class AppFixtures extends Fixture
{
    public function load(ObjectManager $manager)
    {
      ...
      $faker = \Faker\Factory::create('fr_FR');
        ...
        $formationDUTInfo->setDescription("Diplome universitaire de technologie informatique");

        $formationLPProg->setNom("LP Prog avancée");
        $formationLPProg->setDescription("Licence professionnel programmation avancée");

        $formationDUTIC->setNom("DUT TIC");
        $formationDUTIC->setDescription("Diplome universitaire des technologies de l'information et de la communication");
                ...
                //Création des stages
                        foreach($tabEntreprises as $stageEntr)
                        {
                            ...
                            for ($i=0; $i < 10; $i++)
                            {
                                ...
                                $stage->setAdresseMail("contact".$faker->firstname()."@".$stageEntr->getsite());
                                $stage->setMission($faker->realtext(255));
